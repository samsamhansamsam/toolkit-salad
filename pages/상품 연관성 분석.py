...
def run_product_analysis():
    ...
    if uploaded_file is not None:
        ...
        if selected_product_identifier:
            selected_product_name = selected_product['상품명']
            related_products = find_related_products(selected_product_name)
            st.write(f"{selected_product_name}와(과) 함께 구매된 상품:")
            
            if related_products:
                df_related = pd.DataFrame(related_products, columns=['연관 상품', '함께 구매된 횟수'])
